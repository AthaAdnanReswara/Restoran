@extends('layouts.app')

@section('title', 'Edit Pegawai')

@section('content')
<section class="mt-4 sm:mt-6 px-3 sm:px-4 md:px-6 max-w-full sm:max-w-2xl md:max-w-3xl mx-auto pb-24">

    <!-- Header -->
    <div class="mb-6">
        <h2 class="text-xl sm:text-2xl font-bold flex items-center gap-2">
            ✏️ Edit Pegawai
        </h2>
        <div class="h-1 w-20 sm:w-24 bg-yellow-500 rounded mt-2"></div>
        <p class="text-sm sm:text-base text-gray-600 mt-2">
            Perbarui data pegawai sesuai kebutuhan.
        </p>
    </div>

    <!-- Card -->
    <div class="bg-white rounded-2xl shadow-sm border p-4 sm:p-6">

        <form action="{{ route('admin.pegawai.update', $pegawai->id) }}" method="POST"
            class="space-y-4 sm:space-y-5">
            @csrf
            @method('PUT')

            <!-- Nama -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">
                    Nama
                </label>
                <input type="text" name="name"
                    value="{{ old('name', $pegawai->name) }}" required
                    class="w-full rounded-xl border-gray-300 px-4 py-2.5
                           focus:border-yellow-500 focus:ring-yellow-500">
            </div>

            <!-- Email -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">
                    Email
                </label>
                <input type="email" name="email"
                    value="{{ old('email', $pegawai->email) }}" required
                    class="w-full rounded-xl border-gray-300 px-4 py-2.5
                           focus:border-yellow-500 focus:ring-yellow-500">
            </div>

            <!-- Password -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">
                    Password <span class="text-gray-400">(opsional)</span>
                </label>
                <input type="password" name="password"
                    placeholder="Kosongkan jika tidak ingin diubah"
                    class="w-full rounded-xl border-gray-300 px-4 py-2.5
                           focus:border-yellow-500 focus:ring-yellow-500">
            </div>

            <!-- Action -->
            <div class="flex flex-col-reverse sm:flex-row gap-3 sm:justify-between pt-4">
                <a href="{{ route('admin.pegawai.index') }}"
                    class="inline-flex justify-center items-center rounded-xl
                           border border-gray-300 px-5 py-2.5 text-gray-700
                           hover:bg-gray-100 transition">
                    ← Kembali
                </a>

                <button type="submit"
                    class="inline-flex justify-center items-center rounded-xl
                           bg-yellow-500 hover:bg-yellow-600
                           px-6 py-2.5 text-white font-semibold transition">
                    Update Pegawai
                </button>
            </div>

        </form>
    </div>

</section>
@endsection